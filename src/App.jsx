...
function App() {
  ...
  const [lockedColors, setLockedColors] = useState([false, false, false, false, false]); // Lock status for each color
  const [notification, setNotification] = useState({ show: false, message: '', type: 'success' }); // Notification system
  ...
  const generateNewPalette = useCallback(async () => {
    ...
    try {
      ...
      if (hasLockedColors && palette.length === 5) {
        ...
        newPalette = await fetchPaletteWithLockedColors(palette, lockedColors, currentModel);
      } else {
      ...
      const fallbackPalette = palette.length === 5 
   ...
   */
  const copyColor = useCallback(async (color, index) => {
    try {
    ...
    <div className="app">
      ...
      <div className="app-container">
        ...
        <footer className="app-footer">
          <p>
            Made with ❤️ using <a href="http://colormind.io/" target="_blank" rel="noopener noreferrer">Colormind API</a>
          </p>
          <p>
